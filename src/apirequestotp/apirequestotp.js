...
function generateUniqueSuffix() {
  return Math.floor(Math.random() * 1000000); // Angka acak 6 digit
}
...
const users = new SharedArray('userData', () =>
  open('../user.csv')
    ...
    .map(line => {
      ...
      return {
        email: `${email.trim().split('@')[0]}_${uniqueSuffix}@${email.trim().split('@')[1]}`,
        serverKey: serverKey.trim(),
        clientId: clientId.trim(),
        userid: `${userid.trim()}_${uniqueSuffix}`,
      };
    })
);
...
if (__ITER % 5 === 0 && __ITER !== 0) {
  ...
  const payload = JSON.stringify({
    ...
    data: {
        otpType : 1,
        menu : "signing",
        documentName : "doc_fay"
    }
  });

  const res = http.post('https://apionprem.mesign.id/api/v1/otp/requestotp', payload, {
    headers,
    timeout: '180s',
  });
